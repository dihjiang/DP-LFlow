...
# Load the dataset
_, val_set, test_set = set_datasets(args.dataset, None)
INPUT_SHAPE = (3, 32, 32)
if args.test_flag:
...
if args.MLACC_flag:
    ...
    syn_train_loader = DataLoader(get_u_array(syn_train), batch_size=200, shuffle=True, worker_init_fn=_init_fn)
    real_test_loader = DataLoader(get_u_array(real_test), batch_size=200, shuffle=False, worker_init_fn=_init_fn)

    print('==========CNN===========')
    if args.dataset == 'celeba':
        model = NNClassifier(INPUT_SHAPE, type='CNN', data_mode='syn', dataset=args.dataset)
        model = NNfit(model, syn_train_loader, lr=0.001, num_epochs=args.cnn_epochs, data_mode='syn')
        print('==> syn data:')
        evalNN(model, real_test_loader, data_mode='syn', dataset='celeba')
